...
async function main() {
  ...
  if (networkName === "sepolia") {
    nativePayment = parseBool(process.env.SEPOLIA_VRF_NATIVE_PAYMENT) ?? deploymentConfig.vrf.sepolia.nativePayment ?? true;
  } else if (networkName === "mainnet") {
    nativePayment = parseBool(process.env.MAINNET_VRF_NATIVE_PAYMENT) ?? deploymentConfig.vrf.mainnet.nativePayment ?? true;
  } else {
    nativePayment = parseBool(process.env.VRF_NATIVE_PAYMENT) ?? deploymentConfig.vrf.localhost.nativePayment ?? true;
  }
